...
// use [g++ -lstdc++fs -std=c++20 -o editor editor.cpp -lstdc++fs -lraylib -lGL -lm -lpthread -ldl -lrt -lX11] to compile
...
#include "../sf_engine/functions.hpp"
//#include "image_editor.hpp"

#define RAYGUI_IMPLEMENTATION
#include "raygui.hpp"                 // Required for GUI
    ...
    iii = 0;
    if (iii < 128){
        for (const auto & entry : std::filesystem::directory_iterator(working_path)){
        ...
        BeginDrawing();
            ...
            if(counter<255){
                ...
                if(state == 0){
                    ...
                    x_text_offset += GetMouseWheelMove()*(GetFPS()/4);
                    if(x_text_offset > 0){
                        x_text_offset = 0;
                    }
                        }else{
                    ...
                    if(txt_cnt != -1){
                        ...
                        */
                        ...
                        //int txt_cnt = -1;
                    DrawText("Better use outside editor like vscode or notepad++. [Eperimental function]", (GetScreenWidth()*0.12), (GetScreenHeight()-gloabal_font_size-5), gloabal_font_size, RED);
                    }
                }else if(state == 1){
